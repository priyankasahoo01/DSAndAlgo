...
public class FindRepeatMissingNum {
    ...
    public int[] repeatedNumber(final int[] arr) {
        //sum of n num = 5*6/2 = 15,      1,2,3,4,5
        int n = arr.length;
        /*int sum = n*(n+1)/2;
        for(int i : arr){
            sum = sum - i;
        }
        int difff = Math.abs(sum);
        int prod = 1;
        for(int i : arr){prod = prod*i;}*/
        String s= "fdgf";
        s.su
        int prod2 =1;
        for(int i =1; i <=n;i++){prod2=prod2*i;}
        int prod1=1;
        for(int i : arr) {prod1=prod1*i;}
       
        
        
        
        int a1 = Math.min(prod1,prod2);
        int a2 = Math.max(prod1,prod2);
